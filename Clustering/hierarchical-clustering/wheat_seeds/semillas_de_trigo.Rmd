...
```{r, echo=FALSE}
# Haciendo el programa parametrizable con la entrada del usuario.
# Nóteses que como el experimento fue hecho con parámetros muy específicos,
# se le pregunta si quiere o no replicar el experimento.

# El problema está en que de seguir con sus propios parámetros y luego querern
# hacer el knit del archivo (al ser un Rmd), se generarán los comentarios
# respectivos al experimento comentado en el reporte

tamanio_etiquetas <- 0.1
comparativa <- "y"
reescalar <- "y"

# Código para ver si se quiere parametrizar o no el programa con entrada en la consola.
replicar_exp <- readline(prompt="Replicar el experimento (y/n): ")
if (replicar_exp == "n" || replicar_exp == "no") {
  s = readline(prompt="Parametrizando el experimento... ")
  tamanio_etiquetas <- readline(prompt = "Tamaño de etiquetas para gráficas: ")
  tamanio_etiquetas <- as.double(tamanio_etiquetas)
  comparativa <- readline(prompt = "Llevar a cabo análisis comparativo (y/n)")
  reescalar  <- readline(prompt = "Reescalar las características (y/n)")
}
...
distancias <- dist(datos.escalados)
  ...
  distancias <- dist(datos.caracteristicas)
}
```
...
plot(hc.c, xlab="Observaciones",ylab="Altura",main="Dendograma con Distancias máximas",sub="",cex = tamanio_etiquetas,hang = -1)
```
...
# Graficando luego de etiquetar los ejes y reducir el tamaño de las etiquetas de observaciones.
plot(hc.a,xlab="Observaciones",ylab="Altura",main="Dendograma con Distancias máximas",sub="",cex = tamanio_etiquetas,hang = -1)
```
...
```{r}
if (comparativa == "y" || comparativa == "yes") {
...
plot(hc.w, xlab="Observaciones",ylab="Altura",main="Dendograma con Ward",sub="",cex = tamanio_etiquetas,hang = -1)
```
...
En este caso sí hubo un resultado comparativo distinto, pues hubo un menor desajuste entre ambas ténicas en cuando a la diferenciación de las observaciones consideradas para el primero y tercer clusters.