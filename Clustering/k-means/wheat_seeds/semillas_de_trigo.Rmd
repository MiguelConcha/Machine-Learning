...
comparativa <- "y"
reescalar <- "y"
semilla <- 80  # Preferimos que en ésto sí se tenga que cambiar manualmente por comportamiento indefinido que pueda tenerse.
k_inicial <- 3
algoritmo <- "Hartigan-Wong" # Por defecto
...
if (replicar_exp == "n" || replicar_exp == "no") {
  print("Procedemos a parametrizar el experimento...")
  ...
  k_inicial <- readline(prompt = "Número de clusters: ")
  k_inicial <- as.integer(k_inicial)
  print("Los posible algoritmos son: ")
  print("Hartigan-Wong")
  print("Lloyd")
  print("Forgy (igual que Lloyd)")
  print("MacQueen")
  algoritmo <- readline(prompt = "¿Qué algoritmo quieres? (cadena): ")
}
```
...
View(datos.escalados)
  ...
  datos.escalados <- datos.caracteristicas
}
```
...
resultados <- kmeans(datos.escalados, centers = k_inicial, algorithm = algoritmo)
```
...
```{r}
# Creando la tabla con los promedios para cada categoría de observación.
